...
def perform_dqc(config, dqc):
    ...
    for rule in config['rules']:
        ...
        if expectation == 'columns_to_exist':
            ...
            merged_df_new = pd.concat([merged_df_new, df_exists], ignore_index=True)
            merged_df_new['run_dat'] = datetime.datetime.now().strftime("%d/%m/%Y")     
            merged_df_new['missing_count'] = 0 
            merged_df_new['missing_percent'] = 0     
            merged_df_new['unexpected_percent_nonmissing'] = 0    
            merged_df_new['partial_unexpected_index_list'] = []     
            merged_df_new['partial_unexpected_counts'] = []
